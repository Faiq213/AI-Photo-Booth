...
const suggestions = [
  "as an astronaut on the moon",
  "as a knight in shining armor",
  "driving a sports car at sunset",
  "as a cyberpunk character in a neon city"
];

export const SceneDescriptionInput: React.FC<SceneDescriptionInputProps> = ({ description, onDescriptionChange }) => {
  return (
    <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6 flex flex-col items-center text-center h-full w-full shadow-lg">
      <h2 className="text-2xl font-bold text-white mb-2">2. Describe Your Scene</h2>
      <p className="text-slate-400 mb-4">Be creative! Describe the outfit, action, and background.</p>
      <div className="relative w-full h-full min-h-[200px] sm:min-h-0 sm:aspect-square bg-slate-900/70 rounded-lg flex items-center justify-center border-2 border-dashed border-slate-600 focus-within:border-indigo-500 transition-all duration-300 group overflow-hidden">
        <textarea
          ...
          placeholder="e.g., 'me as an astronaut on the moon', or 'a photo of me as a knight in shining armor'"
